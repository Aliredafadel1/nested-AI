...
### User Story 1 — One-Click Demo Login as "Lara" (Priority: P1)

A presenter clicks "Try Demo" on the login page. They are instantly logged in as a pre-seeded student persona named Lara — with a complete profile, saved listings, a conversation history, and a landlord review already posted — without typing any credentials.

**Why this priority**: Demo reliability during a presentation is non-negotiable.

**Independent Test**: Click "Try Demo" → land on the feed as Lara → all 8 user stories are demonstrable without additional setup.

**Acceptance Scenarios**:

1. **Given** the login page, **When** "Try Demo" is clicked, **Then** Lara's JWT is issued and the feed loads with her pre-populated data within 2 seconds.
2. **Given** Lara's session, **When** each of the 8 user stories is exercised, **Then** every feature responds with realistic data (no empty states, no loading errors).
3. **Given** a demo session ends, **When** Lara logs out, **Then** her seed data is not modified (demo is read-heavy, not destructive).
...
- What if the Lara persona's seed data was accidentally modified by a previous session?
...
- **FR-001**: A "Try Demo" button MUST exist on the login page that issues Lara's JWT without a password form.
- **FR-002**: Lara's seed data MUST be resettable with a single command (for pre-demo reset).
...
- **SC-002**: All 19 features from the brief are demonstrable via Lara's persona.
...
- A pre-demo reset command exists to restore Lara's seed data if needed.
