...
**Scope**: Day 4. BGE-M3 embedding pipeline, listing embeddings, student profile embeddings, roommate compatibility engine. Requires Phase 1 complete.
...
- Q: How are the 5 roommate dimension scores derived from embeddings? → A: Option C — 5 separate BGE-M3 sub-embeddings per student, one per dimension field group (sleep, study, cleanliness, guests, budget). Each sub-embedding is a `vector(1024)` stored as a dedicated column in `student_profiles`. Dimension score = cosine similarity between the two students' matching sub-embedding vectors.
...
- Q: How are BGE-M3 model weights sourced? → A: Option A — auto-download from HuggingFace Hub (`BAAI/bge-m3`) on first `worker_init`, cached to a named Docker volume (`bge_model_cache`) so subsequent restarts are instant. Production image (Phase 4) may bake weights in for offline demo reliability.
...
- Q: Which listing fields are concatenated as BGE-M3 input text? → A: Option B — `"{title}. {description}. Neighbourhood: {neighbourhood_name}. Amenities: {amenities_keys_joined}"`. Numeric fields (price, bedrooms) are excluded — handled by SQL filters, not semantic search.
...
When a landlord posts a listing (Phase 1), a Celery task embeds it in the background using BGE-M3. The 1024-dim vector is stored in `listings.embedding`. The student never waits for this — it happens asynchronously.
...
**Independent Test**: Post a listing → wait for `embed_listing` Celery task → verify `listings.embedding` is a 1024-dim vector and is not null.

**Acceptance Scenarios**:

1. **Given** a new listing is created, **When** the `embed_listing` Celery task runs on `nestai:low`, **Then** `listings.embedding` is populated with a non-null `vector(1024)`.
2. **Given** BGE-M3 is already loaded at worker startup, **When** `embed_listing` is called, **Then** the model is NOT reloaded — it reuses the worker-level instance.
...
After the student completes the 8-question onboarding (Phase 3 UI, but the API is built here), their preferences are embedded into a 1024-dim `student_profiles.embedding` vector (full-profile) **and** 5 per-dimension sub-embedding vectors. These vectors power roommate matching.
...
1. **Given** a student submits onboarding data, **When** the `embed_profile` Celery task runs, **Then** `student_profiles.embedding` and all 5 dimension vectors (`dim_sleep`, `dim_study`, `dim_cleanliness`, `dim_guests`, `dim_budget`) are non-null `vector(1024)`.
...
- BGE-M3 worker hasn't initialized yet when the first embed task arrives.
...
- **FR-001**: BGE-M3 MUST load once via Celery `worker_init` signal in `core/embeddings.py`. Never per-request, never per-task invocation.
- **FR-002**: All embeddings MUST be exactly 1024 dimensions. Any mismatch MUST raise immediately, not silently truncate.
- **FR-003**: `embed_listing` MUST use the distributed lock `lock:embed:{listing_id}` (30s TTL) to prevent duplicate embeds.
- **FR-004**: Embedding results MUST be cached in Redis (`embed:{text_hash}`, 48h TTL) to avoid redundant BGE-M3 calls for identical text.
- **FR-005**: `nestai:low` queue MUST be used for embedding tasks (1 worker, 1GB memory cap — BGE-M3 is memory-heavy).
...
- **FR-007**: `embed_profile` Celery task MUST produce 6 vectors per student: 1 full-profile embedding (`student_profiles.embedding`) + 5 dimension sub-embeddings (`dim_sleep`, `dim_study`, `dim_cleanliness`, `dim_guests`, `dim_budget`), all `vector(1024)`. Each `dim_*` column needs a corresponding HNSW index.
- **FR-008**: Null preference fields MUST use the fallback string `"unspecified"` as BGE-M3 input for that dimension — never skip or NULL a `dim_*` column after onboarding.
...
- **FR-011**: The text fed to BGE-M3 for a listing MUST follow the template: `"{title}. {description}. Neighbourhood: {neighbourhood_name}. Amenities: {amenities_keys_joined}"`. Price, bedrooms, and address are excluded from the embedding input.

## Success Criteria

- **SC-001**: `pytest tests/test_embeddings.py -v` — dimension (1024), normalize, batch efficiency, all 6 profile vector fields populated tests all pass.
...
- **SC-004**: BGE-M3 model is loaded exactly once per worker process (verified via worker startup log).
...
- BGE-M3 weights auto-download from HuggingFace Hub (`BAAI/bge-m3`) on first worker startup; cached to the `bge_model_cache` Docker volume. Requires internet access on first run.
- pgvector HNSW index exists on `listings.embedding` and `student_profiles.embedding` (created in Phase 1 migrations).
- `student_profiles` table requires 5 additional `vector(1024)` columns (`dim_sleep`, `dim_study`, `dim_cleanliness`, `dim_guests`, `dim_budget`) added via migration in this phase.
